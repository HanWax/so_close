...
class CompareTimelines
	...
	def initialize(timeline_a: timeline_a, timeline_b: timeline_b, current_user: current_user, neighbour: neighbour, outer_limit: 0.2, inner_limit: 0.02)
	...
	def register_miss(distance_between, timeslot_start)
		...
		@misses <<Miss.create(distance: distance_between, time: timeslot_start, user_id: @current_user.id, neighbour_id: @neighbour.id)
		# puts @misses.length
	end
